...
static TaskHandle_t handleScreenTaskHandle;
...
void redrawList(int page) {
	const int PLAYLIST_DISP_LINES = 4;
	const int PLAYLIST_DISP_COLUMNS = 2;

	xprintf("redraw...\n");
	BSP_LCD_SetBackColor(LCD_COLOR_WHITE);
	BSP_LCD_SetFont(&Font24);

	int itemToDisp = page * 8;
	int posX, posY;
	for (int column = 0; column < PLAYLIST_DISP_COLUMNS; column++) {
		for (int line = 0; line < PLAYLIST_DISP_LINES; line++) {
			BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
			posX = column * 242 + 5;
			posY = line * 55 + 15;
			BSP_LCD_FillRect(posX - 5, posY - 15, 238, 52);
			if (itemToDisp < playListLen) {
				BSP_LCD_SetTextColor(LCD_COLOR_DARKBLUE);
				if (itemToDisp == currentSongIndex)
					BSP_LCD_SetBackColor(LCD_COLOR_YELLOW);
				BSP_LCD_DisplayStringAt(posX, posY,
						(uint8_t*) &fileListBuf[itemToDisp][0], LEFT_MODE);
				BSP_LCD_SetBackColor(LCD_COLOR_WHITE);
				itemToDisp++;
			}
		}
...
void draw_background(void) {
	BSP_LCD_SelectLayer(LCD_LAYER_BG);
	BSP_LCD_FillRect(0, 52, 480, 3);
	BSP_LCD_FillRect(0, 107, 480, 3);
	BSP_LCD_FillRect(0, 162, 480, 3);
	BSP_LCD_FillRect(0, 217, 480, 3);
	BSP_LCD_FillRect(238, 0, 4, 272);
	BSP_LCD_FillRect(77, 220, 3, 272);
	BSP_LCD_FillRect(157, 220, 4, 272);
	BSP_LCD_FillRect(319, 220, 4, 272);
	BSP_LCD_FillRect(400, 220, 3, 272);
	...
	BSP_LCD_FillRect(95, 225, 20, 42);
	BSP_LCD_FillRect(122, 225, 20, 42);

	Point pointD = { 223, 225 };
	Point pointE = { 223, 267 };
	Point pointF = { 176, 246 };
	Point points2[] = { pointD, pointE, pointF };
	BSP_LCD_FillPolygon(points2, 3);
	BSP_LCD_FillRect(176, 225, 10, 42);

	Point pointG = { 257, 225 };
	Point pointH = { 257, 267 };
	Point pointI = { 304, 246 };
	Point points3[] = { pointG, pointH, pointI };
	BSP_LCD_FillPolygon(points3, 3);
	BSP_LCD_FillRect(295, 225, 10, 42);

	BSP_LCD_FillRect(340, 241, 42, 10);
	BSP_LCD_FillRect(356, 225, 10, 42);

	BSP_LCD_FillRect(420, 241, 42, 10);

	playListLen = saveSongsAndReturnAmount("", FILE_LIST_MAX_LEN,
	FILE_NAME_LEN);
...
void startMusic(int index) {
	...
	case PLAYER_STATE_STOPPED:
	case PLAYER_STATE_EOF: {
		saveSongNameToBuffer(index, songNameBuffer);
		playerStartSong(songNameBuffer);
		break;
	}
	}
...
void putcursor(void) {
    xprintf("received %d %d\n", cursorX, cursorY);
	if (cursorUpd) {
		if (0 <= cursorX && cursorX <= 238 && 0 <= cursorY && cursorY <= 52) {
			startMusic(page * 8 + 0);
		} else if (0 <= cursorX && cursorX <= 238 && 55 <= cursorY
				&& cursorY <= 107) {
			startMusic(page * 8 + 1);
		} else if (0 <= cursorX && cursorX <= 238 && 110 <= cursorY
				&& cursorY <= 162) {
			startMusic(page * 8 + 2);
		} else if (0 <= cursorX && cursorX <= 238 && 165 <= cursorY
				&& cursorY <= 217) {
			startMusic(page * 8 + 3);
		} else if (0 <= cursorX && cursorX <= 77 && 220 <= cursorY
				&& cursorY <= 272) {
			switch (player_state) {
			case PLAYER_STATE_PAUSED:
				xprintf("player had been paused - resuming\n");
		...
		} else if (80 <= cursorX && cursorX <= 157 && 220 <= cursorY
				&& cursorY <= 272) {
			switch (player_state) {
			case PLAYER_STATE_PLAYING:
		...
		} else if (161 <= cursorX && cursorX <= 238 && 220 <= cursorY
				&& cursorY <= 272) {
			playPreviousItem();
		} else if (242 <= cursorX && cursorX <= 480 && 0 <= cursorY
				&& cursorY <= 52) {
			startMusic(page * 8 + 4);
		} else if (242 <= cursorX && cursorX <= 480 && 55 <= cursorY
				&& cursorY <= 107) {
			startMusic(page * 8 + 5);
		} else if (242 <= cursorX && cursorX <= 480 && 110 <= cursorY
				&& cursorY <= 162) {
			startMusic(page * 8 + 6);
		} else if (242 <= cursorX && cursorX <= 480 && 165 <= cursorY
				&& cursorY <= 217) {
			startMusic(page * 8 + 7);
		} else if (242 <= cursorX && cursorX <= 319 && 220 <= cursorY
				&& cursorY <= 272) {
			playNextItem();
		} else if (323 <= cursorX && cursorX <= 400 && 220 <= cursorY
				&& cursorY <= 272) {
			if (volume < 100) {
				volume += 10;
				BSP_AUDIO_OUT_SetVolume(volume);
			}
		} else if (403 <= cursorX && cursorX <= 480 && 220 <= cursorY
				&& cursorY <= 272) {
			if (volume > 20) {
				volume -= 10;
				BSP_AUDIO_OUT_SetVolume(volume);
...
//
//void handle_screen(void* pvParameters) {
//    TouchData touchData;
//
//    for (;;) {
//        BSP_TS_GetState(&TS_State);
//        if (TS_State.touchDetected) {
//            touchData.x = TS_State.touchX[0];
//            touchData.y = TS_State.touchY[0];
//            xprintf("%d %d\n", touchData.x, touchData.y);
//            xQueueSend(touchQueue, &touchData, 0); // Send touch data to the queue
...
void StartDefaultTask(void const *argument) {
	...
	if (BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_HEADPHONE1, volume,
	AUDIO_FREQUENCY_44K) == 0) {
...
//    // Create the touchQueue
//        touchQueue = xQueueCreate(5, sizeof(TouchData));
//
//        // Create and start the handle_screen task
//        xTaskCreate(handle_screen, "HandleScreenTask", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, &handleScreenTaskHandle);
//
...
//            HAL_IWDG_Refresh(&hiwdg);
//
//            // Check if there is touch data in the queue
//            if (xQueueReceive(touchQueue, &receivedTouchData, 0) == pdTRUE) {
//                // Use the received touch data
//                cursorX = receivedTouchData.x;
//                cursorY = receivedTouchData.y;
//                cursorUpd = 1;
//                putcursor();
//            }
//
